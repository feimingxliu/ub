...
`memory.Append(workspaceRoot, scope, text)` MUST 在对应文件末尾追加一段形如:
...
(标题前一空行、标题后一空行)。父目录不存在时 MUST mkdir(0o755)。文件不存在时 MUST 创建(0o644)。返回写入的文件绝对路径与本次条目的标题字符串(便于工具 result 引用)。
