...
TUI SHALL 在消息列表中展示 Agent activity 事件，包括 provider thinking、工具生命周期、权限审批结果和错误/notice 摘要。活动流 MUST 使用紧凑前缀与用户消息、assistant 正文区分；活动内容 MUST 按终端宽度换行，并随聊天区历史一起滚动。TUI MUST NOT 把 provider 未返回的隐藏推理链展示为 thinking。同一轮连续 thinking delta MUST 合并为一个可展开 thinking 区域，并在折叠摘要中展示累计可读片段。工具 lifecycle activity MUST 合并为独立的可展开 tool 区域；同一个 tool call 的 queued/running/done/failed 更新 MUST 合并到 tool 区域的同一活动行。权限 activity MUST 展示在 tool 区域内。
...
#### Scenario: thinking delta 累积展示

- **GIVEN** TUI 在同一 Agent turn 中连续收到多个 `thinking` activity delta
- **WHEN** TUI 渲染消息列表
- **THEN** 输出 MUST 合并这些 delta，而不是只保留最后一个碎片
- **THEN** 展开该 activity 后 MUST 能看到累计 thinking 内容
...
- **GIVEN** TUI 在同一 Agent turn 中收到 thinking activity 和 tool activity
- **WHEN** TUI 渲染消息列表
- **THEN** thinking activity MUST 出现在独立可折叠区域
- **THEN** tool activity MUST 出现在独立可折叠区域
