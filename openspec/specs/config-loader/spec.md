...
Define how `ub` discovers, loads, merges, displays, and documents YAML configuration.
...
配置系统 SHALL 支持全局、profile、approval agent 和 provider model 级别的 reasoning 相关配置。`reasoning.effort` MUST 接受 `none`、`minimal`、`low`、`medium`、`high`、`xhigh`。provider model 配置 MAY 声明 `supports_reasoning`、`supported_efforts`、`default_effort` 和 `max_context_tokens`，用于覆盖内置模型能力表和模型上下文窗口。JSON Schema MUST 覆盖这些字段。
