...
> 状态：当前实现对齐（2026-06-12）。V1 已作为历史基线完成，当前主动演进入口见 `roadmap-v2.md`。
...
| Tools | read / write / edit / multiedit（含 diff 预览）、bash（权限审批与流式输出）、grep / glob / ls、job_run / job_output / job_kill、tool_result、ask、task、remember / recall、web_search / web_fetch、plan_write / plan_update / plan_update_step、todo_write / todo_update、LSP 工具、MCP 工具 |
...
- F-PERM-1：每个工具声明 `RiskLevel`：`safe`（read/ls/grep/glob/ask/plan/todo/memory/LSP/task/tool_result 等）/ `write`（write/edit/multiedit）/ `exec`（bash/job_run/job_kill）/ `network`（web_search/web_fetch）
...
| V2（深化） | `roadmap-v2.md` | 已落地：ask、task、memory、plan artifact、todo、multiedit、tool streaming、扩展 LSP、tool_result、web_search/web_fetch、模型发起 plan mode、full-access 等；仍在规划：事件总线、tracing、provider state/prefix cache、sidecar tool process、prompt builder 深化 |
