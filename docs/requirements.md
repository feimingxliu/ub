...
> 状态：当前实现对齐（2026-07-09）。V1 已作为历史基线完成，当前主动演进入口见 `roadmap-v2.md`。
...
- skills 用户自定义系统（hook runner 已有运行时入口，但不作为 V1 插件生态）
- 语音、远程会话、AI 伴侣 UI
...
- F-PROV-2：每个 provider 实现统一接口：`Name()`、`Caps() Caps`、`Chat(ctx, request) -> stream`；支持模型相关能力的 provider MAY 实现 `CapsForModel(model)`，用于 context window、reasoning、prompt cache 等模型级差异
- F-PROV-3：运行时可通过 `/provider [provider] [model]`、`/model [model]` 切换主对话 provider/model，会话上下文保留，切换结果写入当前 session 元数据
...
- F-TOOL-1：工具以接口形式注册（`Name() string`、`Description() string`、`Schema() *jsonschema.Schema`、`Execute(ctx, args json.RawMessage) Result`、`Risk() RiskLevel`）
...
- F-TOOL-9：系统 MUST 提供 goal 工具用于长任务连续执行：`create_goal(objective, token_budget?, turn_budget?)` 创建 session-scoped active goal，`get_goal()` 返回目标、状态和预算/用量，`update_goal(status, block_reason?)` 仅在目标完成、阻塞或暂停时更新状态。goal state MUST 存在 ub state root 下并与当前 session 绑定；host MAY 在 active goal 未终止时自动注入续跑 prompt。goal MUST NOT 替代 plan/todo：goal 管跨 turn 目标和停止条件，plan 管持久方案 artifact，todo 管当前执行清单。
...
- F-MCP-2：启动时根据配置注册 MCP server 工具，连接失败不阻塞主流程；连接可按运行时生命周期关闭，并在后续调用或体检路径中重新探测
...
- F-TUI-1：主界面：聊天区（80%）+ 状态栏（model / effort / mode / context used/max/% / cwd；存在 active goal 时展示 goal 状态）
...
- F-TUI-5：命令：`/provider`、`/model`、`/approval-model`、`/small-model`、`/effort`、`/mode`、`/compact`、`/init`、`/plans`、`/plan-edit`、`/doctor`、`/retry`、`/rewind`、`/btw`、`/goal`、`/copy`、`/clear`、`/new`、`/help`、`/config`、`/profile`、`/sessions`、`/resume`、`/quit`、`/exit`；`/provider` 可在 TUI 内切换当前主对话 provider，并可同时指定目标 model；不指定 model 时 MUST 优先保留目标 provider 可用的当前 model，否则选择目标 provider 的默认/配置模型；切换结果 MUST 写回当前 session 元数据；`/small-model [model]` MUST 只切换当前进程内 auto memory 使用的当前 provider 模型，不改变主对话 model、compact summary model，也不写回配置文件；无参数时 MUST 展示候选列表，显式指定时 MUST 校验候选模型；`/compact` 主动压缩当前 session 上下文，默认使用当前主对话模型生成摘要；`/init` MUST 启动一轮普通 agent 任务来调研当前 workspace 并创建或改进 `AGENTS.md`，已有文件 MUST 尽量保留准确人工内容并通过常规 edit/write 工具产生可见过程和 diff；`/plans` MUST 展示当前 workspace 的 plan artifact 或按 plan id 打开 artifact，`/plan-edit <plan-id>` MUST 用外部编辑器打开指定 plan；`/doctor` MUST 在 TUI 内运行健康检查；`/retry` MUST 重跑上一条 user turn；`/rewind [turn]` MUST 打开或定位历史 user message 选择器，回到目标消息之前并把该消息放回输入框；`/copy [N]` MUST 复制最新回答或 transcript 中指定编号消息；`/goal [objective|clear]` MUST 展示当前 goal、清除当前 session goal 或创建 active goal 并启动 goal-oriented agent turn；`/btw [question]` MUST 打开独立的内存 BTW 视图，带 question 时启动一次独立无工具旁路模型请求，复用当前会话上下文的 text-only 摘要和 BTW 视图内已有 Q/A 作为临时 side history，但不写入主 transcript、conversation history 或 rollout，且运行中不得取消或排队到当前 Agent turn；BTW 请求 MUST 不携带主历史中的 tool_use/tool_result 块，且 provider tool-call 事件或伪工具调用标记 MUST 按错误处理；BTW 视图内 MUST 支持继续输入追问、按普通助手消息规则 Markdown 渲染回答、复制最新答案、清空当前 BTW 记录，并使用独立滚动状态查看长输出；BTW 底部 MUST 使用专属状态行而非主对话状态栏，并以 `answering` / `idle` 展示模型回答中或空闲；`Esc` MUST 退出 BTW 视图并清空该视图的临时 Q/A 与草稿；`/clear` 只清空当前聊天区显示；`/new` 创建并切换到新的空 session，同时清空本地消息、排队输入和 context 状态栏；`/sessions` 可切换当前 workspace 的历史 session 并支持 `search <query>` 搜索历史事件；`/resume` 只用于恢复历史 session，无参数打开 session picker，带 session id 时直接恢复；`/effort` 只允许选择当前模型支持的思考等级；`/profile` MUST 展示 profile 需要重启生效的提示；`/help` MUST 同时列出 slash 命令、输入前缀、键盘快捷键、picker/permission 快捷键和复制相关行为
...
V1 路线图来源 [`roadmap.md`](./roadmap.md) 已完成并作为历史存档；当前主动演进的权威来源是 [`roadmap-v2.md`](./roadmap-v2.md)。这里仅保留版本到 Sprint 的映射，避免双源漂移：
...
| V2（深化） | `roadmap-v2.md` | 已落地：ask、task、memory、plan artifact、todo、multiedit、tool streaming、扩展 LSP、tool_result、web_search/web_fetch、模型发起 plan mode、full-access、goal mode、prompt section/inspect、上下文窗口 resolver、分阶段 ContextDecision/语义裁剪和最小 provider CachePlan；当前下一步：eval MVP，随后基于数据扩展 prompt/context，并进入事件总线/tracing |
