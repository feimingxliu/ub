...
- 想用 OpenAI 兼容端点（DeepSeek / Together / vLLM 等）：对应服务的 API Key 与 base_url
- 想用本地 Ollama：本机已运行 `ollama serve`
...
- Provider base_url 连通性（OpenAI / Anthropic / Ollama / 兼容端点）
...
- **`ub doctor` 报 base_url 不可达**：检查 `ollama serve` 是否启动；检查 `curl <base_url>/models`（OpenAI 兼容）或 `curl <base_url>/api/tags`（Ollama）是否能响应；网络代理是否生效。
