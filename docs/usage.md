...
- workspace memory:`.ub/memory.md` 与全局 memory,见 §13
...
1. **进 plan 模式**(`Shift+Tab` 循环模式,或 `--mode plan` 启动),让 agent 把思路 `plan_write` 成一个 markdown:
   ...
   `plan_write` 只在 plan 模式暴露和执行;work/auto 模式不会让模型创建新 plan。plan 模式会给模型注入规划约束,并隐藏 `write` 风险工具;如果模型误调写工具,仍会被 mode gate 拦截。工具会在 `<workspace>/.ub/plans/<时间戳>-<slug>.md` 写一个文件,并把 `plan_id` 返回。你可以直接打开 review、改步骤(只要保留 `- [ ] N. <text>` 行的格式)。
