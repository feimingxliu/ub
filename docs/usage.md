...
| `ub eval --task <name-or-path>` | 在隔离 workspace/session 中运行真实 coding-agent 行为评测；重复 task/target 可生成 matrix |
...
重复 `--task` 或使用 `--target provider=model`、`--repeat`、`--parallel` 会进入 matrix 模式。每个样本继续完全隔离，原始结果按 target→task→repetition 稳定排序；报告同时按 overall/target/task 汇总样本数、通过率、失败分类、耗时和 token/cache。每个 target 的首个样本兼作 preflight：若在任何工具或 assistant 行为前发生 provider/config/protocol infrastructure failure，该 target 的剩余样本会直接标记 skipped；行为/断言失败不会熔断。`--target` 不能与旧 `--provider`/`--model` 混用，最大并发为 16，默认 1。
