...
1. **进 plan 模式**(`Shift+Tab` 循环模式,或 `--mode plan` 启动),让 agent 把思路 `plan_write` 成一个 markdown:
   ...
   `plan_write` 只在 plan 模式暴露和执行;work/auto 模式不会让模型创建新 plan。plan 模式会给模型注入规划约束,并隐藏 `write` 风险工具;如果模型误调写工具,仍会被 mode gate 拦截。工具会在 `$XDG_STATE_HOME/ub/plans/<project-key>/<时间戳>-<slug>.md` 写一个文件,并把 `plan_id` 和绝对 `path` 返回。你可以直接打开 review、改步骤(只要保留 `- [ ] N. <text>` 行的格式)。
...
3. **中断恢复**:下次会话从上次 tool result / rollout 中取回 `path` 后可直接 `read` 这个 state-root 下的 plan,或用 `plan_update_step(plan_id="...")` 继续标记进度。plan artifact 是用户 state 数据,不会写入工作区或参与 git。
