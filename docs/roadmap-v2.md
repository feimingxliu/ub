...
| S2-11 | `/rewind [turn]`:回退到指定历史 turn,默认回退上一轮 user turn(已实现) | M |
| S2-12 | `/btw [question]`:旁路快问快答,不打断当前任务且不污染主对话历史(已实现) | S-M |
...
- **改动面**:
  ...
  - `internal/app/ub/agent/agent.go` `runTool` 路径:为 streaming tool 起 goroutine 转发到 agent event sink
  - `internal/app/ub/tui/`:新增 `EventToolPartialOutput` event 类型;tool 块支持"running + 滚动预览 + done"三态渲染
  - 第一批接入:`bash`(最痛)、`job_output --follow`;未来可接入 `lsp.diagnostics --watch`
...
现状:`internal/app/ub/agent/loop.go` 把 prompt build / tool dispatch / context mgmt / summary 混在一起。
...
2. **Memory 自动归纳的成本/收益调优**:`memory.auto.enabled` 默认开启后,需要继续观察后台批量抽取的触发门槛、误写率、外部上下文跳过策略和用户感知收益
