...
- **当前写入与检索**:
  ...
  - 自动归纳:agent 成功完成 work/auto/full-access turn 后,用 `small_model`(未配置时复用当前模型)提取最多 `memory.auto.max_candidates` 条长期事实;plan 模式不写 memory
...
2. **Memory 自动归纳的成本/收益调优**:`memory.auto.enabled` 默认开启后,需要继续观察每 turn 多一次 small_model 调用的成本、误写率和用户感知收益
