...
| §4 架构优化 | prompt / context / eventbus / tracing / provider cache | **当前主战场** | 优先上下文与 prompt/cache |
| §5 生态拓展 | IDE / Web / 协作 | 未启动 | 等社区采用信号 |
...
- 当前主要缺口不再是"补功能",而是上下文窗口/压缩策略、prompt builder 分层、prefix cache、事件/trace 观测、eval 与研究输出。
...
**状态**:未完成,但已具备前置基础。当前已有 `prompt_harness.go`、startup prompt、workspace instructions、git snapshot、structured compact prompt 和相关测试;本条要把这些从 agent 包内字符串/函数推进为可枚举、可检查、可缓存的 section registry。
...
- **现状**:`internal/app/ub/agent` 里 runtime context、execution mode、memory 注入和 summary 模板已经存在,但缺少统一的 section registry 与稳定测试边界
- **目标结构**:
...
- S4-06 Prompt builder 分层:建立 section registry、稳定顺序、snapshot/golden 测试和最小 `ub prompt inspect` 或 `ub doctor --prompt`。
- 上下文窗口预测:当 provider/model 没有显式 `max_context_tokens` 时,根据内置 modelinfo、provider caps、历史超限/usage 反馈推断并 cache 最可信窗口。
- 压缩策略优化:让 summary/compact 明确保留 goal、plan/todo 状态、当前阻塞、已验证事实、用户纠正和剩余风险;继续保持完整 user-turn 边界。
...
- Eval 报告:从 rollout 汇总工具调用模式、首次工具延迟、失败恢复路径、token/cache 经济。
- Internals 文档:补 `docs/internals/rollout-format.md`、`adding-a-tool.md`、`adding-a-provider.md` 中最常被贡献者问到的路径。

### v0.7 Provider & Protocol Experiments
目标:在不破坏 rollout replay 的前提下探索 provider 新能力。
...
1. **上下文窗口预测**:推断结果应按 provider/model 缓存在配置、state 还是 rollout 派生索引?用户显式配置和观测到的 provider 超限错误谁优先?
