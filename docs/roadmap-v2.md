...
> 状态:当前规划对齐 (2026-07-17,当前发布基线 = v0.5.1)。本路线图覆盖 v0.2 ~ v1.0 期间的已交付基线与后续演进候选。
...
| §4 架构优化 | prompt / context / eventbus / tracing / artifact / provider cache | **当前主战场** | 先用 eval 固化 prompt/context 基线,再进入可观测性 |
...
截至 v0.5.1,ub 已经具备一个完整的本地 coding agent 基线:
...
- v0.5.0/v0.5.1 已交付 prompt section registry 与 inspect、ContextWindowResolver、分阶段 ContextDecision/semantic pruning、最小 CachePlan/live probe、plan-mode 语义修正和两分支 auto memory。
- 当前主要缺口不再是"补功能",而是用 eval 固化 prompt/context 行为基线,再扩展 ContextDecision 裁剪规则、tool schema/MCP cache plan、事件/trace 观测与研究输出。
- 旧的 v0.2/v0.3/v0.4 节奏建议保留为历史参考;当前从 `v0.5 Context & Prompt Reliability` 的 eval 收口继续推进。
...
- **改动**:`internal/command/tui.go` / `internal/command/root.go` 写入 / 查询 session 时统一过 `filepath.Abs` + `filepath.EvalSymlinks` + `filepath.Clean`
...
- **当前实现**:
  ...
  - `internal/agent/agent_tool_runner.go` 在 streaming path 中起 goroutine 执行工具,并把 `StreamEvent` 转发为 `EventToolPartialOutput`
  - `internal/tui/` 支持 running partial output 与最终 done/failed 详情合并渲染
...
**状态**:主体已实现。S3-10A 的 coding-agent 指令、workspace `AGENTS.md` 注入、git snapshot、tool guidance 和 structured compact prompt 已交付；原 S3-10B/S3-10C 中的 section registry、`ub prompt inspect`、compact manifest 与最小 provider CachePlan 已分别并入 S4-06/S4-07 落地。当前剩余主项是 §6-03 eval MVP；provider/model 级 prompt profile 保留为有评估依据后再启动的候选。
...
- **后续切片(S3-10B/S3-10C)**:
  - ✅ `ub prompt inspect` 已实现,可展示 main/no-tool/compact manifest、section 状态、来源、截断、token 估算与 cacheable 标记,正文默认脱敏
  - 基于 rollout 增加小型行为评测:read-vs-ls、先读后改、测试失败不报成功、复杂任务更新 todo/plan、压缩后继续任务
  - provider/model 级 prompt profile 暂不实现；先由 eval 证明小模型与大模型确实需要不同工具指导
- **配置**:
...
**状态**:已实现。新增 `enter_plan_mode` / `exit_plan_mode` 安全工具;work 模式由模型请求后直接进入 plan,plan 模式带 `plan_id` 展示精确 artifact 并请求用户批准退出,再恢复本进程 `pre_plan_mode`;auto/full-access 默认不广告入口。mode 切换写入独立 `mode` activity 和 rollout 审计,缺少、无效或不存在 `plan_id` 的退出请求会直接返回明确 tool error。
...
- **MVP 范围**:
  ...
  - 模型调用后直接切到 `plan`;入口不弹确认框,只读调研和写 plan artifact 的边界由 mode gate 强制,用户只在退出时审批方案
...
- **验收标准**:
  - work 模式下,复杂任务的 fake provider 能调用 `enter_plan_mode`,TUI 直接进入后只暴露 plan 工具,`exit_plan_mode` 展示精确 artifact 并获批准后恢复 work
...
- **实现面**:
  - 更新 `internal/mode.ParseMode` / `Gate`、`internal/config.ValidateExecutionMode`、schema、docs/requirements 和 docs/design 的 mode 枚举
...
**状态**:已完成第一轮包内解耦。原先的单体 agent loop 已拆到 `agent_stream.go`、`agent_tool_runner.go`、`agent_limits.go`、`tool_modes.go`、`activity_*`、`memory_auto.go`、`event_record.go` 等文件;`Agent` 仍保留在 `internal/agent` 包内,通过 `Options` / `Factory` 注入 provider、tool registry、permission、rollout、context、prompt、hook、memory、file history 等外置状态。
...
**状态**:前两条垂直切片已实现。当前已有固定顺序的 prompt section registry，main/read-only/no-tool 构造共用同一模型，并提供默认脱敏的 `ub prompt inspect` 文本/JSON manifest；structured summary 已作为稳定、无工具的 `compact_instructions` section 接入同一 manifest，并支持 `ub prompt inspect --variant compact`。后续切片聚焦 tool schema/MCP tool 的可检查边界与构造性能，不再重复建设 summary inspect。
...
- **现状**:`internal/agent` 已为 coding-agent、runtime、workspace instructions、git snapshot、execution mode、memory 和 compact instructions 建立稳定 section ID、顺序、状态、来源、字符/token 估算和截断元数据；tool schema 与动态 MCP tool 尚未纳入统一 inspect/cache plan
- **目标结构**:
...
- **Reasoning 兼容性修复**：保留模型声明的 `max` 等级；区分未指定与显式 `none`，后者不再回退到默认等级，并向支持 reasoning 的 OpenAI-compatible 模型发送关闭参数。
...
- **现状**:
  - `provider.Caps.SupportsPromptCache` 和 `Usage.CacheReadTokens/CacheWriteTokens` 已存在
  - OpenAI-compatible 已设置 `SupportsPromptCache: true`，`eventUsage` 从 `PromptTokensDetails.CachedTokens` 和 DeepSeek 的 `prompt_cache_hit_tokens` 两种来源解析 cache hit
  - Anthropic `applyCacheBreakpoints` 在 system/tools + conversation tail 设置 `cache_control`，有单测锁定 placement
  - prompt section registry 有 stable/dynamic 标记，`ub prompt inspect` 展示 `cacheable` 标记
  - `ub doctor` 展示各 provider 的 `prompt_cache`（`cache_control` / `cached_tokens`）
- **后续切片**:
  - 用已有 env-gated live probe 持续验证真实 provider 的 cache read/write；运行依赖本地 provider 配置,不作为默认离线测试
  - 把 live probe 的 cache 命中和 reasoning round-trip 结果纳入后续 eval/report,避免只保留一次性人工结论
  - 更细的 CachePlan：把 tool schema、MCP tool 等也纳入可缓存性分析
- **验收标准**:
  ...
  - ✅ `ub prompt inspect` 展示 cacheable sections
  - ✅ `ub doctor` 展示 provider cache 支持
  - ✅ live probe 已实现并通过验证：[`internal/provider/livecache_test.go`](https://github.com/feimingxliu/ub/blob/main/internal/provider/livecache_test.go)，用 `UB_LIVE_CACHE_TEST=1 go test ./internal/provider/ -run TestLiveCacheHitProbe` 运行
...
**状态**:首个保守切片已实现,是 v0.5 上下文可靠性的核心基线。请求前 token 估算、手动 compact、provider overflow recovery 已统一进入 `ContextSnapshot -> ContextDecision`，并记录可恢复审计；后续工作是扩展裁剪覆盖面和评估信息保留率。
...
- **当前核心模型**:`ContextSnapshot -> ContextDecision`
  - snapshot 记录请求估算、有效窗口、输出 reserve、完整配对状态、可 compact 边界与 tool-result 候选
  - decision 明确 `keep` / `prune` / `compact` / `compact-and-retry`,并记录 `reason=threshold|manual|overflow|incomplete|mid_turn`
  - rollout 保存 tokens before/after、cut boundary、pruned/protected tool IDs、summary model、耗时、retry 和最终 provider context；活动事件显示脱敏决策摘要
- **当前语义裁剪**:
  - 仅裁剪保留窗口外、完整配对的旧成功 `read` / `grep` 结果：后续相同输入结果覆盖它，或其输出明确为空；裁剪后的实际 estimate 仍超预算时回退 structured compact
  - error、文件写入/验证类工具、当前 turn、保留窗口与不完整配对默认保护；裁剪用同 ID 的占位 result，不删除 provider pairing，完整 transcript/rollout/spillover 不丢原输出；prune checkpoint 仅用于 resume，不重复命中 session search
- **已验证**:
  - planner 单测覆盖 unknown window、threshold、manual、overflow、受保护结果和 mid-turn
  - fake provider 覆盖 prune-first 不调用 summary、原始 transcript 保留和 rollout/context 恢复
  - `ub prompt inspect --variant compact` 可检查 default/short compact 模板且默认脱敏
- **后续候选**:在有评估数据后扩展 job polling、plan/goal 语义保护和任务完成率/关键信息保留率 eval；不引入不透明的通用语义相似度删除。
...
5. Token 估算与自动压缩(`internal/tokenizer`)
...
**状态**:前两条垂直切片已实现。单 task 真实 Agent 评测、确定性 context runtime overrides、task/model matrix、重复采样、有界并发、target preflight/熔断和原始/聚合报告均已落地。

- `docs/eval-tasks/` 首批提供 5 个小任务:源码定位、先读后改、失败验证不报成功、复杂任务更新 todo/plan、压缩后继续任务
- `ub eval --task X --provider P --model Y` 在临时 workspace 和隔离 XDG state/data 中运行当前 executable；task 可用 follow-up prompts 在同一 session 继续
- task schema v1 支持 fixture、文件/命令/rollout 断言，以及受限的 context runtime overrides；fixture 路径拒绝绝对路径、逃逸和 symlink
- runtime overrides 通过隔离子进程参数确定性建立 context/compact 前置条件，不复制 provider 凭据；文本/JSON 报告回显有效覆盖
- matrix 使用重复 `--task` 与显式 `--target provider=model`，支持 `--repeat` / `--parallel`；每个样本保持独立 workspace/session，报告顺序稳定
- target 首样本作为正式 preflight；只有无 agent 行为的 infrastructure failure 会熔断剩余样本，避免把 provider/model 不兼容计作行为失败
- 单次输出包含通过状态、失败分类、耗时、turn、input/output/reasoning/cache token、工具序列和 ContextDecision；matrix 额外按 overall/target/task 汇总样本数、通过率和资源指标，同时保留 raw reports
- **后续**:从 rollout 增加首次工具延迟、审批/工具/compact 耗时和失败恢复路径；有真实多轮基线后再评估置信区间、baseline diff 与 CI threshold
- **价值**:让 ub 自己作为评测脚手架
...
按当前 v0.5.1 基线,后续节奏重新规划如下:

### 当前已交付基线(v0.2.0 ~ v0.5.1)
...
- v0.5.0 ~ v0.5.1:完成 prompt section/inspect、ContextWindowResolver、分阶段 ContextDecision/semantic pruning、最小 CachePlan/live probe,并收口 plan mode 与 auto memory 语义。

### v0.5 Context & Prompt Reliability
目标:把"模型能不能稳定做对"从经验问题变成可观察、可测试、可调优的系统问题。

- S4-06 Prompt builder 分层:section registry、稳定顺序、manifest 测试、`ub prompt inspect` 和 compact manifest 已完成；后续只扩展 tool schema/MCP tool 的可检查边界。
...
- S4-08 ContextDecision + semantic pruning:首个保守切片已统一 threshold/manual/overflow/mid-turn 决策，先裁剪可证明被覆盖或为空的旧 read/grep 结果，并按实际 estimate 回退 structured compact；完整 user-turn 与 tool-call/result 边界、错误和验证类结果受保护，prune checkpoint 不重复命中 session search。后续再基于 eval 扩展 goal/plan/todo 语义保护。
- S4-07 最小 CachePlan:可缓存 section 稳定性、usage 解析、Anthropic cache breakpoint、OpenAI-compatible cache 能力和 env-gated live probe 已完成；更细的 tool schema/MCP CachePlan 等 eval 数据出现后再做。
- §6-03 eval harness 已完成第二切片:首批五个 task、隔离 runner、确定性 runtime overrides、跨模型 matrix/preflight/熔断和原始/聚合报告已建立可重复基线；下一步用重复样本决定 ContextDecision 与 prompt/cache 的扩展。
