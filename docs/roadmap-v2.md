...
**状态**:MVP 已实现。当前提供单 task 的真实 Agent 行为评测；批量任务、model matrix、聚合成功率与统计报告留到 v0.6。
...
- 输出包含通过状态、失败分类、耗时、turn、input/output/reasoning/cache token、工具序列和 ContextDecision；`--json` 提供机器可读对象，`--keep-workspace` 保留现场
- **后续**:增加批量 runner、跨 model/provider matrix、重复运行与成功率/方差报告，再从 rollout 汇总首次工具延迟、审批/工具/compact 耗时和失败恢复路径
- **价值**:让 ub 自己作为评测脚手架
...
- §6-03 eval MVP 已完成:首批五个 task、隔离 runner、文件/命令/rollout 断言和文本/JSON 报告已建立可重复基线；下一步先运行跨模型样本,再决定 ContextDecision 与 prompt/cache 的下一轮扩展。
