...
- 当前主要缺口不再是"补功能",而是扩展 ContextDecision 的裁剪规则、prompt builder 后续分层、prefix cache、事件/trace 观测、eval 与研究输出；上下文窗口 resolver 与保守的 ContextDecision 首个切片已落地。
...
- **现状**:`internal/agent` 已为 coding-agent、runtime、workspace instructions、git snapshot、execution mode、memory 建立稳定 section ID、顺序、状态、来源、字符/token 估算和截断元数据;summary 模板和 tool schema 尚未纳入统一 inspect/cache plan
- **目标结构**:
...
- **后续切片**:
  - 基于真实 provider usage 验证 cache read/write（目前只有单测）
  - 增加 env-gated live probe，验证 DeepSeek repeated prefix hit 与 reasoning round-trip 风险
