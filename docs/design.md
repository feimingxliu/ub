...
`plan_write` / `plan_update_step`:把 plan-then-execute 工作流落到磁盘。plan 模式产出 `.ub/plans/<id>.md`(标题、metadata、`## Steps` 任务列表、`## Notes`、`## Log`),work/auto 模式按这个 artifact 推进并 `plan_update_step` 标记每一步。`plan_write` 只在 plan 模式暴露和执行;work/auto 下即使 provider 误调用也返回错误。两者都是 `RiskSafe`(写的是 ub 内部 artifact 目录,不是用户代码)。
