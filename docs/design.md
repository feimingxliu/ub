...
| V2 深化 | — | `roadmap-v2.md` | 已落地 ask、task、memory、plan/todo、multiedit、tool streaming、扩展 LSP、tool_result、web 工具、模型发起 plan mode、full-access、goal mode、prompt section/inspect、上下文窗口 resolver、分阶段 ContextDecision/语义裁剪和最小 provider CachePlan；当前下一步是 eval MVP，随后基于数据扩展 prompt/context，并进入事件总线/tracing |
