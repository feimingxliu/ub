...
完成后从隔离 rollout 汇总 turn、usage/cache token、tool result 序列和 summary 的 ContextDecision action/reason，再联合文件、验证命令、assistant 文本断言生成报告；报告同时回显规范化 runtime overrides。默认文本输出；`--json` 输出单个稳定对象。失败按 `task` / `agent` / `infrastructure` / `timeout` / `assertion` / `internal` 分类并返回非零状态；子进程失败后 runner 会 best-effort 读取已经产生的隔离 rollout，使“完全没有 agent 行为的请求/配置失败”和工具执行后的 agent failure 可以区分。

Matrix 模式复用同一个单次 runner。重复 `--task`、显式 `--target provider=model`、`--repeat > 1` 或 `--parallel > 1` 会进入 matrix；target 显式配对而不是自动生成 provider/model 交叉积。planner 以 target→task→repetition 生成稳定 run ID，每个样本仍创建自己的临时 workspace/state/data/session；worker pool 只限制并发，最终 raw results 按计划顺序输出。每个 target 的第一个样本兼作正式 preflight，只有在没有 tool call/assistant 行为的 `infrastructure` failure 时才熔断该 target，其余计划项以 skipped + 根因保留。单 task、非显式 target、repeat=1 继续输出旧 `Report`。

`MatrixReport(kind=matrix)` 保留所有执行样本的完整 `Report` 和 skipped 记录，并按 overall/target/task 汇总 planned/executed/passed/failed/skipped、failure category、pass rate（分母为 executed）、duration/turn、token/cache 与 ContextDecision action。默认文本输出摘要和失败/skip；`--json` 输出完整稳定对象。任一 failed 或 infrastructure-triggered skipped 返回非零状态。
...
| V2 深化 | — | `roadmap-v2.md` | 已落地 ask、task、memory、plan/todo、multiedit、tool streaming、扩展 LSP、tool_result、web 工具、模型发起 plan mode、full-access、goal mode、prompt section/inspect、上下文窗口 resolver、分阶段 ContextDecision/语义裁剪、最小 provider CachePlan，以及带确定性 runtime/matrix/聚合报告的 eval harness；下一步扩展 eval 指标并进入事件总线/tracing |
