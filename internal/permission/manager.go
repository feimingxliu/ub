...
func (m *Manager) Ask(ctx context.Context, req Request) (Result, error) {
	parsedMode, err := execmode.ParseMode(string(req.Mode))
	if err != nil {
		return Result{}, err
	}
	req.Mode = parsedMode
	if err := execmode.Gate(parsedMode, req.Risk); err != nil {
...
// which must pass through rules/approval regardless of execmode. Safe and write
