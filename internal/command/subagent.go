...
func (r *cliSubagentRunner) currentMode() execmode.Mode {
	if r == nil {
		return execmode.ModeWork
	}
	if r.modeFunc != nil {
		if mode, err := execmode.ParseMode(string(r.modeFunc())); err == nil {
			return mode
		}
	}
	if mode, err := execmode.ParseMode(string(r.mode)); err == nil {
		return mode
	}
	return execmode.ModeWork
}
