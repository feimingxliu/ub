...
func TestPermissionModalPreservesMultilineDraft(t *testing.T) {
	...
	req := PermissionRequest{
		Request:  permission.Request{Tool: "bash", Risk: tool.RiskExec, Mode: execmode.ModeWork},
		Response: response,
	}
...
func TestPlanModePromptApproveEnterSwitchesMode(t *testing.T) {
	...
	select {
	case got := <-response:
		if !got.Approved || got.FromMode != execmode.ModeWork || got.ToMode != execmode.ModePlan {
...
func TestNextExecutionModeIncludesFullAccess(t *testing.T) {
	cases := []struct {
		...
		{current: string(execmode.ModeWork), want: string(execmode.ModePlan)},
		{current: string(execmode.ModePlan), want: string(execmode.ModeAuto)},
		{current: string(execmode.ModeAuto), want: string(execmode.ModeFullAccess)},
		{current: string(execmode.ModeFullAccess), want: string(execmode.ModeWork)},
	}
	for _, tc := range cases {
...
func TestStreamEventsContinueDuringPermissionModeSwitch(t *testing.T) {
	...
	updated, _ = model.Update(permissionRequestMsg{request: PermissionRequest{
		Request:  permission.Request{Tool: "bash", Risk: tool.RiskExec, Mode: execmode.ModePlan},
		Response: response,
	}, ok: true})
	model = assertModel(t, updated)
...
func (r *scriptedRunner) EnterPlanMode() (string, string, error) {
	from := defaultString(r.mode, string(execmode.ModeWork))
	if from != string(execmode.ModePlan) {
		r.prePlanMode = from
	}
	r.mode = string(execmode.ModePlan)
...
func (r *scriptedRunner) ExitPlanMode() (string, string, error) {
	from := defaultString(r.mode, string(execmode.ModeWork))
	to := r.prePlanMode
	if to == "" || to == string(execmode.ModePlan) {
		to = string(execmode.ModeWork)
	}
	r.mode = to
	r.prePlanMode = ""
	return from, to, nil
