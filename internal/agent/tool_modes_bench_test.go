...
func BenchmarkToolDefinitionsWorkMode(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		defs, err := toolDefinitions(reg, execmode.ModeWork)
		if err != nil {
			b.Fatal(err)
		}
		if len(defs) == 0 {
...
func BenchmarkAgentToolDefinitionsCachedWorkMode(b *testing.B) {
	reg := benchmarkToolRegistry(b)
	a := &Agent{tools: reg, toolDefinitionCache: map[execmode.Mode][]provider.ToolDefinition{}}
	if _, err := a.toolDefinitions(execmode.ModeWork); err != nil {
		b.Fatal(err)
	}
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		defs, err := a.toolDefinitions(execmode.ModeWork)
		if err != nil {
			b.Fatal(err)
		}
		if len(defs) == 0 {
